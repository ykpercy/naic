...
def Encoder(x,feedback_bits):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        ...
        # x = layers.Dense(units=int(feedback_bits/B))(x)
...
def Decoder(x,feedback_bits):
    ...
    x = layers.Conv2D(256, 3, padding = 'SAME',activation="relu")(x_ini)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(256, 3, padding = 'SAME',activation="relu")(x)
    x = layers.BatchNormalization()(x)
    ...
    x = layers.BatchNormalization()(x)
    x_ini = keras.layers.Add()([x_ini, x])  # 线性残差连接


    decoder_output = layers.Conv2D(2, 3, padding = 'SAME',activation="sigmoid")(x_ini)
    # decoder_output = layers.Conv2D(2, 3, padding = 'SAME',activation="relu")(x)
